...
<div class="bdy col-span-12 lg:col-span-9 2xl:col-span-10">
	...
	<div
		...
		<div class="w-full sm:w-auto flex">
			<button class="btn btn-primary shadow-md mr-2" id="restoreEmail">복구</button>
			<button class="btn btn-warn shadow-md mr-2" id="emailDelBtn">영구
				삭제</button>
			<div class="dropdown">
				...
				<nav class="w-full sm:w-auto sm:mr-auto">
					<ul class="pagination">
						...
						<li class="page-item"><a class="page-link"
							href="/emp/mail/trash?currentPage=1"> <svg
									xmlns="http://www.w3.org/2000/svg" width="24" height="24"
									viewBox="0 0 24 24" fill="none" stroke="currentColor"
									stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
						...
						<li class="page-item"><a class="page-link"
							...
							href="/emp/mail/trash?currentPage=${data.startPage-5}&size=${param.size}&cond=${param.cond}&keyword=${param.keyword}">
								<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
									viewBox="0 0 24 24" fill="none" stroke="currentColor"
									stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
									icon-name="chevron-left"
		            	...
		            	">
								<a
								href="/emp/mail/trash?currentPage=${pNo}&size=${param.size}&cond=${param.cond}&keyword=${param.keyword}"
						...
						<li class="page-item"><a class="page-link"
							<c:if test='${data.endPage == data.totalPages}'>style="pointer-events: none;"</c:if>
							href="/emp/mail/trash?currentPage=${data.totalPages}"> <svg
									xmlns="http://www.w3.org/2000/svg" width="24" height="24"
									viewBox="0 0 24 24" fill="none" stroke="currentColor"
									stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
	...
	$(function() {
		...
		$("#emailDelBtn").on("click", function(){
			...
			if(checkMailList.length > 0){
				let answer = confirm(checkMailList.length +"건의 메일을 영구삭제 처리하시겠습니까?");
				if(answer){
					jQuery.ajax({
						url:"/emp/mail/inbox/clearEmailList",
		...
		$("#restoreEmail").on("click", function(){
			...
			if(checkMailList.length > 0){
				let answer = confirm(checkMailList.length +"건의 메일을  복구하시겠습니까?");
				if(answer){
					jQuery.ajax({
						url:"/emp/mail/trash/restoreEmailList",
 						contentType: 'application/json; charset=utf-8',
						data: JSON.stringify(checkMailList),
						type:"post",
						dataType:"json",
